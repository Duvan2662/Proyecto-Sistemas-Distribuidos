...
@Module({
  imports: [
    TypeOrmModule.forRoot({
      ...
      database: 'biblioteca_ingenieria',
      entities: [Libro, Sede, Prestamo, Usuario],
      synchronize: true,
      logging: false,
    }),
    TypeOrmModule.forFeature([Libro, Sede, Prestamo, Usuario], 'ingenieria'),
  ],
  ...
  providers: [
    {
      ...
      useFactory: (libroRepo, sedeRepo, prestamoRepo) =>
        new SedeService(libroRepo, sedeRepo, prestamoRepo, 'ingenieria'),
      inject: [
        getRepositoryToken(Libro, 'ingenieria'),
        getRepositoryToken(Sede, 'ingenieria'),
        getRepositoryToken(Prestamo, 'ingenieria'),
      ],
    },
  ],
})
